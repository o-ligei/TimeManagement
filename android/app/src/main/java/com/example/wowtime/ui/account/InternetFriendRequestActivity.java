    ...
    private void OKGetInternetFriends(){
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Request request = new Request.Builder().url(InternetConstant.host + "/Social/GetFriendRequest").post(formBody.build()).build();
                try {
            ...
            public void run() {
                ...
                for(int i = 0; i < jsonArray.length(); i++){
                    ...
                    try {
                        ...
                        listItem = new InternetFriendItem(Integer.valueOf(item.get("userId").toString()),String.valueOf(item.get("userIcon")),String.valueOf(item.get("username")));
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                    list.add(listItem);
                }
                InternetFriendRequestItemAdapter friendsListAdapter = new InternetFriendRequestItemAdapter(list, getApplicationContext());
